...
% Keep _02+_03, figures only (no Excel)
out = ma1_analyze_reaches_session( ...
...
- Eye-cal runs (`effector==0` only) auto-excluded; `skip_runs` is filename stem and/or `_xx` (not folder order)
- `write_excel` default `true`; `'write_excel', false` skips `.xlsx`
